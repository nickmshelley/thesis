   ...
   (map 
    ...
    (append
     (parameterize ([current-namespace (make-base-empty-namespace)])
       ...
       (with-handlers ([(λ (x) #t) (λ (e) (printf "Ignoring exception for lang ~a~n~a~n~n" lang e))])
     ...
     (parameterize ([current-namespace (make-base-empty-namespace)])
       ...
       (parameterize ([current-load-relative-directory (build-path (current-directory) base)])
         (for ([require requires])
           (with-handlers ([(λ (x) #t) (λ (e) (printf "Ignoring exception for require ~a~n~a~n~n" require e))])
     ...
     (parameterize ([current-namespace (make-base-empty-namespace)])
       ...
       (cond
         [(not (empty? structs))
          ...
          (for ([a-struct structs])
            (with-handlers ([(λ (x) #t) (λ (e) (printf "Ignoring exception for struct ~a~n~a~n~n" a-struct e))])
              ((compose eval read open-input-string) a-struct)))
          (filter-not (λ (v)
                        (member v base))
                      (namespace-mapped-symbols))]
         [else empty]))))
