\documentclass[ms,electronic,letterpaper,oneside,chaptercenter,parttop,lof]{byumsphd}
\usepackage{slatex}
\usepackage{graphicx}
\usepackage[
    bookmarks=true,
...
\newcommand{\Title}{Autocompletion Without Static Typing}
\newcommand{\Author}{Nick Shelley}
\newcommand{\GraduationMonth}{July}
...
This thesis provides two validation techniques that provide information both on how well completion suggestions are ordered and also which completion suggestions result in errors.
...
\acknowledgments{%
    Thanks to my wife and Dr. McCarthy for their support and patience.
}
...
\paragraph{Thesis Statement} Automated testing frameworks for completion systems of dynamically-typed languages reveal information and patterns that enable more informed and targeted improvements to such systems.
...
Static type information makes it relatively simple for completion systems for such languages to filter out invalid results.
...
Because validation efforts for such completers are minimal, improvements to such systems rely on user complaints, and implementing new systems correctly is hard at best.
We show how automated testing frameworks help to alleviate these problems.
...
Jungloid mining~\cite{Jungloid} uses intimate knowledge about the Java type system, library and API functions, example code, and the current context of the program to suggest ways to get from one type to another type through intermediate steps.
...
Another paper takes abbreviated input instead of prefixes and uses a hidden Markov model to determine what was meant~\cite{Abbreviated}.
For example, the expression \texttt{ch.opn(n)} would turn into \texttt{chooser.showOpenDialog(null)}.
Another paper tracks all changes to the codebase and prioritizes suggestions based on how recently methods and objects have been changed~\cite{History}.
One paper uses existing code (such as the Eclipse code base) as training data to implement three different completion systems~\cite{Examples}.
...
For instance, one rule might be to suggest setters on objects after they are created.
...
Most validation techniques run their systems on modified existing code and rank the results in some way.
One paper~\cite{Abbreviated} runs their system on 3000 lines of code from 6 open source projects that were turned into acronym-like abbreviations.
They measure how many lines could be completed and how many of those are in the top-N (i.e. top-1 or top-5) suggested completions.
Hou and Pletcher~\cite{Towards} run their completion system at every Swing method in several open source projects.
They rank how high in the suggestions list the actual method appear and add all rankings together to get an overall ranking for the system.
Another paper~\cite{History} runs their completion system between every recorded change operation (defined as a change in the AST) on a project of theirs called SpyWare.
They then record what percentage of actual methods were in the top-N suggestions.
Lastly, another paper~\cite{Examples} uses the Eclipse code base to randomly split the code into 90\% training data and 10\% test data.
They then take out half of the Swing method calls in the test data and run their completion system in each hole.
They measure recall, which is how often a used method was in the suggestion list, and precision, which is what fraction of suggested methods were actually used.
They then combine these two measures into an F1-measure to give an overall grade to a system.

Several papers use methods that rely heavily on static information and context that is not feasible to get in Racket because of the reasons described in the introduction.
...
A few papers use methods that are orthogonal to this thesis, but could be starting points for potential future improvements.
Calcite~\cite{Calcite} suggests popular ways to instantiate a given class or interface based on analysis of existing code.
...
The lack of testing done on dynamic auto-completers helps emphasize the need for a validation system for such languages.
Most testing is done by comparing completion results of modified existing source to the original source.
The metrics they use are all similar and help define metrics in a way that works best for such validation systems.
Lastly, although a lot of improvements are type-system specific, some methods work without static type information and are used as starting points for methods implemented in this thesis.
...
We predict that the second case is easier to complete against because there is more information.
...
For a given token in a source file, the ranker modifies the file at that position, either by removing or truncating as described above, and then invokes an auto-completer to get a list of completion results.
...
One obvious source of potential completion options is from the source file itself.
...
We then determine whether the position of the completer invocation is in function application position by checking if the position is directly after an opening parenthesis.
...
\caption[Ranking results]{Ranking results. Ranked refers to completions in the top five positions. Missed means they weren't in the results at all.}
