...
Last, much more is known in the literature about the first set of methods, whereas little has been done to show how automated testing can help completers for languages such as Racket.
...
The first way is to truncate the source file from the chosen identifier, which represents writing new source code.
...
For the sake of time and simplicity, we choose n to be 5.
...
For language keywords, a regular expression was used to find all function names based on the \scheme{define} and \scheme{let} keywords.
...
For both \scheme{#lang} and \scheme{require}, we find macros of this kind using a regular expression and then \scheme{dynamic-require} them into a new empty namespace.
...
However, because there is no pre-defined form for dynamically creating structs, it is necessary to \scheme{eval} the resulting forms in a new namespace.
...
Racket source files can be expanded and compiled to byte code with \scheme{zo-parse}.
...
The ensuing investigation led to the discovery and fixing of a bug in the regular expression used to find function names in the \scheme{let} form.
